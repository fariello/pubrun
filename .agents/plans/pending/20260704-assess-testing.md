...
| 2 | TST-02 | Add `test_tree_rss_linux`: mock `/proc/<pid>/statm` and `/proc/<pid>/task/<pid>/children` filesystem reads. Verify sum includes self + children + grandchildren. | `tests/test_resources.py` | Low | Platform-skip on non-Linux; covers lines 65-94 |
| 3 | TST-03 | Add `test_tree_rss_darwin`: mock `subprocess.check_output` for `ps -eo pid,ppid,rss` with synthetic tree. Verify correct parse and sum. | `tests/test_resources.py` | Low | Platform-skip on non-macOS (or mock); covers lines 104-148 |
